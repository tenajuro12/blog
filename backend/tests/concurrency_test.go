...
// ── TC-CONC-01: 10 goroutines register simultaneously ────────────────────────
// Goal: verify no 500 panics under concurrent writes to SQLite.
// SQLite serializes writes internally — some goroutines may get lock errors (non-500).
func TestConcurrency_SimultaneousRegistrations(t *testing.T) {
	...
	for i, code := range results {
		if code == http.StatusInternalServerError {
			t.Errorf("goroutine %d: got 500 — server panicked", i)
		}
	...
	t.Logf("Concurrent registrations: %d/%d succeeded", successes, n)
}

// ── TC-CONC-02: Double like in 2 goroutines ───────────────────────────────────
// Goal: DB must have exactly 1 like, no 500 errors.
func TestConcurrency_DoubleLike(t *testing.T) {
	...
	if count > 1 {
		t.Errorf("expected max 1 like in DB after race, got %d", count)
	}
	t.Logf("Double like: results=%v DB_count=%d", results, count)
}

// ── TC-CONC-03: Double bookmark in 2 goroutines ───────────────────────────────
func TestConcurrency_DoubleBookmark(t *testing.T) {
	...
	if count > 1 {
		t.Errorf("expected max 1 bookmark in DB after race, got %d", count)
	}
	t.Logf("Double bookmark: results=%v DB_count=%d", results, count)
}

// ── TC-CONC-04: Double follow in 2 goroutines ────────────────────────────────
func TestConcurrency_DoubleFollow(t *testing.T) {
	...
	if count > 1 {
		t.Errorf("expected max 1 follow in DB after race, got %d", count)
	}
